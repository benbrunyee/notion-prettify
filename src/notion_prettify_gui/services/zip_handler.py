...
class ZipHandler:
    """Resolves the actual Export-*.zip that notion-export-prettify expects.
    ...
    "Export-", this handler extracts it and locates the inner Export-*.zip.
    """
    ...
    def resolve(self, input_path: Path) -> Path:
        ...
        If *input_path* is already an Export-*.zip or not a zip at all, it is
        returned unchanged.  Otherwise the zip is extracted and the first
        Export-*.zip found inside is returned.
        """
        if input_path.suffix.lower() != ".zip":
            return input_path

        if input_path.name.startswith("Export-"):
            return input_path
    ...
    def _extract_and_find_export_zip(self, outer_zip: Path) -> Path:
        ...
        matches = list(extract_dir.rglob("Export-*.zip"))
        if not matches:
            raise ZipExtractionError(
                f"No 'Export-*.zip' found inside '{outer_zip.name}'. "
